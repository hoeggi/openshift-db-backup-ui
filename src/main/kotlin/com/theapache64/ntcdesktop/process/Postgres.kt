...
import com.theapache64.ntcdesktop.DUMP
...
class Postgres {
    ...
    suspend fun dumpDatabase(databse: String, path: String, password: String) = withContext(Dispatchers.DUMP) {
        PostgresResult.Dump(
            ProcessBuilder(dump(databse, path))
    ...
    companion object {
        ...
        fun dump(databse: String, path: String) = listOf(
            "pg_dump",
            "--user=postgres",
            "--host=localhost",
            "--port=5432",
//            "--file=${File(path, "$databse-${LocalDateTime.now().toString().replace(" ", "-")}")}",
